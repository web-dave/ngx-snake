...
@Component({
    selector: 'snake-game', templateUrl: './game.component.html', styleUrls: ['./game.component.scss'],
})
export class GameComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    startGame() {
        ...
        fromEvent<KeyboardEvent>(document.body, 'keyup')
            ...
            }), map((event) => event.code), map((code) => code.replace('Arrow', '').toLowerCase() as IDirection), filter((code) => ['right', 'left', 'up', 'down'].includes(code)))
            .subscribe((code) => {
                console.log(code);
                this.direction = code;
            });
    ...
    setFootPosition() {
        this.food = {
            x: Math.floor(Math.random() * this.cols), y: Math.floor(Math.random() * this.rows),
        };
    }
    ...
    drawSnakeFace(pos: IPosition) {
        ...
        // Half Rect
        this.ctx.fillRect((pos.x * this.pxlSize) + (this.direction === 'left' ? halfSize : 0), (pos.y * this.pxlSize) + (this.direction === 'up' ? halfSize : 0), (this.direction === 'left' || this.direction === 'right' ? halfSize : this.pxlSize), (this.direction === 'left' || this.direction === 'right' ? this.pxlSize : halfSize));
        this.ctx.beginPath();
        ...
        // Eye
        this.ctx.arc((pos.x * this.pxlSize) + (this.direction === 'left' || this.direction === 'right' ? halfSize : quarterSize), (pos.y * this.pxlSize) + (this.direction === 'left' || this.direction === 'right' ? quarterSize : halfSize), 3, 0, Math.PI * 2, false);
        this.ctx.stroke();
    }
    ...
    drawFinished() {
        ...
        if (this.user) {
            this.hallOfFameService.hallOfFameControllerAddScoreEntry({
                body: {
                    score: points,
